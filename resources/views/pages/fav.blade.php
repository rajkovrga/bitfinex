...
@endsection
@section('content')
    @if(\Illuminate\Support\Facades\Redis::get('types') != null)
        <input type="hidden" name="types" value="{{\Illuminate\Support\Facades\Redis::get('types')}}">
    @endif
    <div class="flex flex-col items-center">
        <H1 class="text-2xl py-10">Favorites</H1>
        @if(session('error'))
            {{ session('error') }}
        @endif
